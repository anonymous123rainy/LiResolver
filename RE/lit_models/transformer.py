...
class BertLitModel(BaseLitModel):
    ...
    def _init_label_word(self, ):
        ...
        label_path = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))+'/'+f"dataset/{model_name_or_path}_{dataset_name}.pt"
    ...
    def pvp(self, logits, input_ids):
        ...
        #! hard coded
        xxx = (input_ids == self.tokenizer.mask_token_id)
        #print('xxx', type(xxx), xxx.shape)
        _, mask_idx = xxx.nonzero(as_tuple=True)
        ...
        mask_output = logits[torch.arange(bs), mask_idx] ### shape mismatch: indexing tensors could not be broadcast together with shapes
